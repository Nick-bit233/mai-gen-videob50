...
`v1.0` 测试版本现已更新：
- ✨ **重大更新**：现已支持中二节奏（CHUNITHM）B30视频生成！并统一支持了水鱼查分器和落雪查分器的双数据源更新
...
- 您需要下载两个压缩包，运行环境包和本体分离，进行以下操作：
    - 下载本体压缩包和运行环境包（`runtime_v<版本号>.zip`），并解压
    - 将运行环境包中的**全部文件**，复制到本体包解压后的目录
    - 双击`start.bat`文件启动应用。
...
> 如果你具有基本的计算机和python知识，可以独立（或者LLM辅助）完成环境配置和脚本操作，可以直接clone仓库代码，参考[使用说明](#使用说明（从源代码启动）)部分开始使用。
